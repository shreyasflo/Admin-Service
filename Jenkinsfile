node {
	...
	try {
	    ...
	    stage('Build Docker Image') {
	    		sh 'docker build -t shreyasramnath/adminservice:${BUILD_NUMBER}  -f Dockerfile .'
	    		sh 'docker tag shreyasramnath/adminservice:${BUILD_NUMBER} shreyasramnath/adminservice:latest' 
	    }
	        
	    stage('Push Docker Image to Docker Hub') { 
	    		withCredentials([usernamePassword(credentialsId: 'shreyasDocker', passwordVariable: 'pass', usernameVariable: 'user')]) {
	    			sh 'docker login -u $user -p $pass' 
	        		sh 'docker push shreyasramnath/adminservice'
			}	 
	    }
	    
	    stage('Deploy in AWS ECS'){
            sh 'aws ecs update-service --region us-east-1 --cluster idexceldemo --service shreyas-admin --force-new-deployment'
       		sh 'sleep 30'
       		sh 'aws ecs wait services-stable --region us-east-1 --cluster idexceldemo --service shreyas-service'
       	}
	} catch (Exception e) {
